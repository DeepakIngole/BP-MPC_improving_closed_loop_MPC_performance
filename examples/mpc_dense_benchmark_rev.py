#!/usr/bin/env python3
"""Standalone MPC benchmarking script for solve and reverse-mode derivative performance.
...
def run_benchmark(use_prepared: bool, n_warmup: int = 5, n_evals: int = 100):
    print("=" * 60)
    print(f"🚀 Running MPC Benchmark (use_prepared={use_prepared}, diff_mode=rev)")
    print("=" * 60)
    ...
    solver = setup_dense_solver(
        ...
        options={"diff_mode": "rev"}, # <-- Explicitly using reverse mode differentiation
    )
    ...
    # JIT the fast solve target
    fast_solve_jit = jax.jit(fast_solve_fn)

    # For reverse mode differentiation (grad), the function must return a scalar. 
    # We sum the output vector u0 to fulfill this requirement.
    def loss_fn(cs):
        return jnp.sum(full_solve_for_grad(cs))
    ...
    # ======================================================================
    # Benchmarking: Reverse Mode Derivative (Grad)
    # ======================================================================

    solver.timings.reset()

    print("\n[2/2] Benchmarking Grad (Reverse-mode w.r.t cost_state)...")
    t0 = time.perf_counter()
    _ = grad_jit(cost_state0)
    ...
    print(f"  Average execution time: {(t1 - t0) / n_evals * 1000:.3f} ms / grad")
    print(solver.timings.summary())
    print("\n")
