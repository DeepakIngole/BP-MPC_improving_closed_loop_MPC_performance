...
Usage:
    python mpc_sparse_benchmark.py                # Runs with use_prepared = True
    python mpc_sparse_benchmark.py --no-prepared  # Runs with use_prepared = False
"""
...
def run_benchmark(use_prepared: bool, n_warmup: int = 5, n_evals: int = 100):
    print("=" * 60)
    print(f"🚀 Running MPC Benchmark (mode=sparse, use_prepared={use_prepared})")
    print("=" * 60)
    ...
    # Variables for dummy eval and benchmarking
    ...
    # Solver Setup
    # ======================================================================
    # Only supply cost_state to prepare if it's considered a slow variable
    dummy_prep_vars = {"cost_state": cost_state0} if use_prepared else {}
    
    # Extract the full BCOO structure by doing a dummy evaluation
    # (This ensures the matrices have their correct shapes and indices)
    dummy_qp = problem.prepare(dummy_prep_vars)
    ...
    # get sparsity
    sparsity_patterns = problem.sparsity_pattern

    print(f"  Sparsity — "
          f"P: {sparsity_patterns["P"].nse} nnz / {n_dec * n_dec}, "
          f"A: {sparsity_patterns["A"].nse} nnz / {n_eq * n_dec}, "
          f"G: {sparsity_patterns["G"].nse} nnz / {n_ineq * n_dec}")
    ...
    if use_prepared:
        # Pre-compute the slow variables once (we can reuse the dummy evaluation here)
        prepared_qp = dummy_qp
